...
<Container>
  ...
  [//]: #ExamplesStart
    ...
    #### Explanation:
      ...
      2. Two pointers: i = current child, j = current cookie.
      3. While both pointers are in range: if the cookie satisfies the child (s[j] >= g[i]), assign it (count++, move both). Otherwise try the next cookie (j++).
