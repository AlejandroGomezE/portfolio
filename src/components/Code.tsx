  ...
  return (
    <button
      ...
      className={clsx(
        ...
        copied ? 'bg-emerald-400/10 ring-1 ring-inset ring-emerald-400/20' : 'bg-white/5 hover:bg-white/7.5'
      )}
      onClick={() => {
...
function CodePanelHeader({ tag, label }: { tag?: string; label?: string }) {
  ...
  return (
    <div className="flex h-9 items-center gap-2 border-y border-b-white/7.5 border-t-transparent bg-white/2.5 bg-zinc-900 px-4">
      {tag && (
...
function CodePanel({ children, tag, label, code }: { children: React.ReactNode; tag?: string; label?: string; code?: string }) {
...
function CodeGroupHeader({ title, children, selectedIndex }: { title: string; children: React.ReactNode; selectedIndex: number }) {
          ...
          {Children.map(children, (child, childIndex) => (
            <Tab
              className={clsx(
                ...
                childIndex === selectedIndex ? 'border-emerald-500 text-emerald-400' : 'border-transparent text-zinc-400 hover:text-zinc-300'
              )}
            >
...
export function CodeGroup({ children, title, ...props }: React.ComponentPropsWithoutRef<typeof CodeGroupPanels> & { title: string }) {
  let languages = Children.map(children, (child) => getPanelTitle(isValidElement(child) ? child.props : {})) ?? [];
  let tabGroupProps = useTabGroupProps(languages);
  let hasTabs = Children.count(children) > 1;

  let containerClassName = 'not-prose my-6 overflow-hidden rounded-2xl bg-zinc-900 shadow-md';
  let header = (
