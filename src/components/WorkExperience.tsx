...
export default function WorkExperience() {
  return (
    <div className="mt-24 text-gray-500 relative z-10">
      <FadeIn
        variants={{
          hidden: { opacity: 0 },
          visible: { opacity: 1 },
...
function WorkRole({ children, title, date, image }: { children: React.ReactNode; title: string; date?: string; image: { url: string; className: string } }) {
  return (
    <FadeIn className="flex group mt-8 first:mt-0 px-3">
      <div className="hidden md:flex md:flex-col">
        <p className="px-4 pt-8 group-first:pt-0 text-white text-sm leading-7 min-w-[180px] max-w-[180px] md:min-w-[195px] md:max-w-[195px] xl:max-w-[215px] xl:min-w-[215px] flex-none">{date}</p>
        <div className={clsx('relative flex-none rounded-md overflow-hidden self-center mx-4 mt-auto mb-auto', image.className)}>
          <Image fill className="" src={image.url} alt="" />
        </div>
      </div>
      <Border className="pt-8 group-first:pt-0 group-first:before:hidden group-first:after:hidden">
        <div className="flex mb-4">
          <div className={clsx('relative flex-none rounded-md overflow-hidden self-center mx-4 md:hidden', image.className)}>
            <Image fill className="" src={image.url} alt="" />
          </div>
          <div>
            <p className="font-semibold text-work_experience_orange text-lg">{title}</p>
            <p className="md:hidden mt-2 text-white text-sm">{date}</p>
          </div>
        </div>
